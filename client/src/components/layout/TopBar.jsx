...
const TopBar = () => {
  ...
  const openProfile = () => {
    setMenuOpen(false);
    navigate('/settings', { state: { from: location.pathname, tab: 'profile' } });
  };
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div ref={menuRef} className="relative z-[90]">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-2.5 py-1.5 text-left text-sm font-medium text-slate-700 shadow-sm hover:-translate-y-0.5 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
          >
            <img
              src={avatarSrc}
              alt={user?.username || 'User'}
              className="h-8 w-8 rounded-full object-cover ring-2 ring-emerald-100 dark:ring-slate-700"
            />
